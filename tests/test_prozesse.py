"""Die Zuordnung der Regeln zu Geschäftsprozessen.

Die Abdeckungsseite ist ein Leistungsversprechen gegenüber dem Kunden. Sie
darf deshalb nicht stillschweigend unvollständig werden, wenn eine Regel
dazukommt - diese Tests halten die Zuordnung vollständig und widerspruchsfrei.
"""
...
def test_jede_regel_gehoert_zu_einem_prozess(prozesse, katalog):
    """Sonst fällt sie aus der Abdeckungsseite heraus, ohne dass es auffällt."""
...
def test_auch_die_abgeschalteten_regeln_sind_zugeordnet():
    ...
    Sonst verschwände der Katalog sein eigenes Leistungsversprechen, sobald
    ...
    # Über Bereiche und Kategorien zugeordnete Regeln lassen sich hier nicht
    # auflösen; geprüft wird deshalb gegen den vollen Katalog mit Konfiguration.
...
def test_jeder_prozess_nennt_seine_grenzen(prozesse):
    """Wer sagen kann, was nicht geprüft wird, wird beim Rest geglaubt."""
...
def test_jeder_prozess_nennt_seine_schwerpunkte(prozesse):
    ...
    assert not duenn, f"Prozesse mit weniger als drei Prüfschwerpunkten: {duenn}"


def test_kernprozesse_haben_eine_prozesskette(prozesse):
    """Ein durchgängiger Prozess ohne Schritte lässt sich nicht darstellen."""
...
def test_zuordnung_ueber_bereich_kategorie_und_id(prozesse, katalog):
    ...
    assert "VEN-COMP-001" in ids, "Zuordnung über den Bereich greift nicht"
    assert "X-FMT-001" in ids, "Zuordnung über die Regel-ID greift nicht"
    dubletten_ids = {regel.id for regel in prozesse.rules_of(dubletten, katalog.rules)}
    assert "MAT-DUP-002" in dubletten_ids, "Zuordnung über die Kategorie greift nicht"


def test_prozesse_veraendern_die_katalogversion_nicht():
    """Eine geschärfte Formulierung ist kein anderer Maßstab (FA-605)."""
...
def test_fehlende_datei_ist_kein_fehler(tmp_path):
    """Ohne Prozessdatei läuft das Werkzeug weiter, nur die Seite bleibt leer."""
    (tmp_path / "leer").mkdir()
